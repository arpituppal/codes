...
void update(int node,int st,int end,int idx,int val)
{
	...
	seg[node].lsum=MAX(left.lsum,(left.sum+right.lsum));
	seg[node].rsum=MAX(right.rsum,(left.rsum+right.sum));
	seg[node].sum=left.sum+right.sum;
	seg[node].mxsum=MAX(left.mxsum,MAX(right.mxsum, left.rsum+right.lsum));
}
